...
int main()
{
	...
	//Run function cardEffect using village
	...
	//Hand should have 1 more cards than before village was used
	...
	printf("Add 2 Actions (Test Player's Number of Actions Increased Correctly) : ");
	//Hand should have 2 more actions than before village was used
	if(G.numActions == (copyG.numActions + addActions)){
		...
		printf("Test Player Discarded village (Discard Count is Increased Correctly): ");
	...
	printf("Test Player Discarded village (Discard Pile Contains village): ");
	//Varible for the containing village
	int contains = 0;

	//Loop through player discard pile and check for village
	...
	//Check the game state is unaffected by village by compare previous game state to current game state
	
	printf("Game State Remains Unchanged (Other Player's Hand Count is Unchanged): ");
	//check other player's hand count is the same as before village was used
	...
	//check other player's deck count is the same as before village was used
	...
	//check other player's discard count is the same as before village was used
	...
	//Finished Result
	if(overall == 0){
		printf(">>>Testing of village FAILED <<<\n");
	}

	else{
		printf(">>>Testing of village PASSED <<<\n");
	}
